...
sum_wells_4['SAMPLE_NO'] = sum_wells_4['index']+1


# In[95]:

sum_wells_5 = sum_wells_4.drop('index', 1)
sum_wells_5

sum_wells_6 = sum_wells_5.loc[:, ('SAMPLE_NO','SAMPLE_ID', 'BCR_ABL1_COPIES', 'ABL1_COPIES', 'RATIO', '%_RATIO', 'DROPLETS_SUM')]
sum_wells_6
...
with open(infile + "_out.csv", 'w') as f:
    ...
    linewriter.writerow(sum_wells_6.columns)
    linewriter.writerows(sum_wells_6.values)
    linewriter.writerow(" ")
    linewriter.writerow(" ")
...
os.system("cp "+infile.replace(" ", "\ ")+"_out.csv "+outfolder.replace(" ", "\ "))
print(outfolder.replace(" ", "\ "))
#os.system("mv " + res_sname + "/" + barcode + "_rawlib_lowcov_regions.csv " + res_sname + "/" + sname +
#          "_low_cov_regs.csv")
# In[ ]:
